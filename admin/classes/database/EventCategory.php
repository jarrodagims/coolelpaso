...
class EventCategory extends Category{
	...
	public $_moduleCategoryName = 'EventCategory';
	// Static Variables
	protected static $_addLabel = 'Add Event Category';
	protected static $_editLabel = 'Edit Event Category';

	// Inherited Variables - Model
	protected $_dbTable	= 'event_categories';
	protected $_permalinkField = 'permalink';
	protected $_action = 'index';

	// Inherited Variables - Category
	protected $_childClass = 'Event';
	...
	public function setColor($value){$this->_color = (string) $value; return $this;}
	...
	public function setRecords($records = ''){
		if(is_array($records)){
			$this->_events = $records;
		}elseif($this->getId()){
			$records = new $this->_moduleClassName();
			$this->_events = $records->fetchAll("WHERE `category` = ".$this->getId(), "ORDER BY `start_date` DESC");
		}else{
			$this->_events = array();
		}
		return $this;
	...
	public function setEvents($events = ''){
		if(is_array($events)){
			...
			$events = new $this->_moduleClassName();
			if($this->getFeaturedMode()){
				$featuredFilter = " AND `featured` = '1' ";
			}else{
				$featuredFilter = " AND `do_not_show_on_news`='0' ";
			}
			$this->_events = $events->fetchAll("WHERE (`category` = ".$this->getId()." ".$featuredFilter.") AND `post_date` <= NOW() AND (`remove_date` >= CURDATE() OR `remove_date` = '0000-00-00')","ORDER BY `date` DESC");
		}else{
			$this->_events = array();
		}
		return $this;
	...
	public function indexAction(){
		$this->setEvents();
		if(sizeof($this->getEvents())){
			ob_start();
			foreach($this->getEvents() as $events){
				$events->setHrefPrefix($this->getHrefPrefix());
				echo $events->toHtml('listing');
			}
			return ob_get_clean();
		}else{
