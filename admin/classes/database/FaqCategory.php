<?php
class FaqCategory extends Category{
	// Category Configuration 
	public $_childClass = 'Faq';
	public $_sortBy = 'sort_order';
	
	// Module Configuration
	public $_moduleClassName = 'Faq';
	
	// Static Variables
	protected static $_addLabel = 'Add FAQ Category';
	protected static $_editLabel = 'Edit FAQ Category';

	// Inherited Variables
	protected $_dbTable = 'faqs_categories';
	...
	public function setRecords($value = ''){return $this->setFaqs($value);}
	public function getRecords(){return $this->getFaqs();}

	public function setFaqs($value = ''){
		if(is_array($value)){
			$this->_faqs = $value;
		}elseif($this->getId()){
			$faq = new $this->_moduleClassName();
			$this->_faqs = $faq->fetchAll("WHERE `category` = ".$this->getId(),"ORDER BY `sort_order`,`question`,`id`");
		}else{
			$this->_faqs = array();
		}
		return $this;
	...
	public function menuAction(){
		...
		ob_start(); ?>
        <ul class="actions">
			<li><a href="<?php echo $child_item->buildModalUrl('add', $this->getId()); ?>" data-toggle="modal" data-target="#moduleModal"><i class="fa fa-plus"></i>Add FAQs Item</a></li>
	...
	public function indexAction(){
		ob_start();
		if(count($this->getFaqs())){
			?>
			<div class="faq_section">
				<div class="faq_category"><?php echo process($this->getName()); ?></div>
				<dl>
