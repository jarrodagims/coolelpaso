...
if($action == 'edit'){
	...
	# Create new instance of module category class
	$moduleCategoryClass = new $moduleCategoryClassName($_POST['id']);
...
if($action == 'delete'){
	...
	if($moduleClass->fetchCount("WHERE `category` = ".$moduleCategoryClass->getId())){
		echo failure('Records still belong to '.$moduleCategoryClass->getName().'.', '', 'messages', array($moduleCategoryClass->_moduleCategoryName,$moduleCategoryClass->getName(),$moduleCategoryClass->getId()));
		exit;
	}
