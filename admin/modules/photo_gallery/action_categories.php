...
if($action == 'delete'){
	...
	if($moduleClass->fetchCount("WHERE `category` = ".$moduleCategoryClass->getId())){
		echo failure('There are still photos in '.$moduleCategoryClass->getName().'.', '', 'messages', array($moduleCategoryClass->_moduleCategoryName,$moduleCategoryClass->getName(),$moduleCategoryClass->getId()));
		exit;
	}
