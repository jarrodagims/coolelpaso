...
(function (factory) {
    ...
    tModalMain = '<div id="' + MODAL_ID + '" class="file-zoom-dialog modal fade" tabindex="-1" aria-labelledby="' +
        MODAL_ID + 'Label"></div>';
    tModal = '<div class="modal-dialog preview-modal-mod" role="document">\n' +
        '  <div class="modal-content">\n' +
        '    <div class="modal-header">\n' +
        '      <div class="kv-zoom-actions pull-right"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button></div>\n' +
    ...
    tActions = 
        ...
        '    </div>\n' +
        '    {drag}\n' +
        '    <div class="checkbox checkbox-custom pull-right hidden">\n' +
		'		<input id="checkbox{dataJustKey}" type="checkbox">\n' +
		'		<label for="checkbox{dataJustKey}"></label>\n' +
        '    </div>\n' +
        '    <div class="file-upload-indicator" title="{indicatorTitle}">{indicatorTitle}</div>\n' +
        '    <div class="clearfix"></div>\n' +
        '</div>';
    //noinspection HtmlUnknownAttribute
    tActionDelete = '<button type="button" class="kv-file-remove btn btn-custom" ' + 'title="{removeTitle}" {dataUrl}{dataKey}><i class="fa fa-times" aria-hidden="true"></i></button>\n';
    tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
        '{uploadIcon}</button>';
    tActionZoom = '<button type="button" class="kv-file-zoom btn btn-custom" title="{zoomTitle}"><i class="fa fa-search" aria-hidden="true"></i></button>';
    tActionDrag = '<span class="file-drag-handle drag-handle-init" title="{dragTitle}">{dragIcon}</span>';
    tTagBef = '<div class="file-preview-frame{frameClass}" id="{previewId}" data-realindex="{dataJustKey}" data-fileindex="{fileindex}"' +
        ' data-template="{template}"';
    tTagBef1 = tTagBef + '><div class="kv-file-content">\n';
    tTagBef2 = tTagBef + ' title="{caption}" ' + STYLE_SETTING + '><div class="kv-file-content">\n';
    tTagAft = '</div>{footer}\n</div>\n';
    tGeneric = '{content}\n';
    //tHtml = '<iframe style="border: 1px solid #ddd;" frameborder="0" class="kv-preview-data" width="{width}" height="{height}" src="https://docs.google.com/viewerng/viewer?embedded=true&url={data}"></iframe>\n';
	tHtml = '';
    tImage = '<img src="{data}" class="kv-preview-data file-preview-image" title="{caption}" alt="{caption}" >\n';
    //tText = '<iframe style="border: 1px solid #ddd;" frameborder="0" class="kv-preview-data" width="{width}" height="{height}" src="{data}"></iframe>\n';
	tText = '';
    ...
    //tPdf = '<iframe style="border: 1px solid #ddd;" frameborder="0" class="kv-preview-data" width="{width}" height="{height}" src="https://docs.google.com/viewerng/viewer?embedded=true&url='+document.location.origin+'{data}"></iframe>\n';
	tPdf = '';
    ...
    FileInput.prototype = {
        ...
        _generatePreviewTemplate: function (cat, data, fname, ftype, previewId, isError, size, frameClass, foot, ind, templ) {
            var self = this, css = frameClass || '', caption = self.slug(fname),id =$(foot).find('.kv-file-remove').data('key'),
                config = ifSet(cat, self.previewSettings, defaultPreviewSettings[cat]), prevContent, zoomContent = '',
                footer = foot || self._renderFileFooter(caption, size, config.width, isError, id),
                ...
                getContent = function (c, d, zoom) {
                    ...
                    return tmplt.replace(/\{previewId}/g, id).replace(/\{caption}/g, caption)
						...
						.replace(/\{dataJustKey}/g, $(foot).find('.kv-file-remove').data('key'))
        ...
        _renderFileFooter: function (caption, size, width, isError, id) {
		...
		addExternal: function (options) {
            var self = this, $el = self.$element;
			self._initUploadSuccess(options)
			self._setFileDropZoneTitle();
			 self.uploadCache = {content: [], config: [], tags: [], append: true};
            return self.$element;
        }
